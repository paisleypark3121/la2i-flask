...
def create_vectordb(location, embedding, persist_directory=None, overwrite=False):
    
    vectordb=None

    if persist_directory and not overwrite and os.path.exists(persist_directory):
...
def create_vectordb_from_content(content,embedding, persist_directory=None, overwrite=False):
